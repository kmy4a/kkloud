...
class VPCs:
    ...
    def delete(self, vpc_id: str) -> None:
        ...
        vpc = VPC(
            ...
            cidr_block=next(vpc['cidr_block'] for vpc in self.vpcs if vpc['id'] == vpc_id),
            vni=int(next(vpc['vni'] for vpc in self.vpcs if vpc['id'] == vpc_id)),
            is_attatched_to_igw=bool(next(vpc['is_attatched_to_igw'] for vpc in self.vpcs if vpc['id'] == vpc_id))
        )
    ...
    def attach_igw(self, vpc_id: str) -> None:
        ...
        self.vpcs = [v if v['id'] != vpc_id else vpc_obj.model_dump() for v in self.vpcs]
        self.save()
